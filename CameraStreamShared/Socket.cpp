...
std::string Socket::getSocketLastError() {
    std::stringstream sstr;
    Socket::outputSocketLastError(sstr);
    return sstr.str();
}
...
Socket& Socket::operator=(const Socket& sref) {
    this->close();
    this->_socket = sref._socket;
    return *this;
}
...
int SocketConnection::recvFrom(char* data, int dataLen, InetHost* from, int flags) const {
    ...
    //unsigned long avaliablePacket
    //ioctlsocket(this->_socket, FIONREAD);
