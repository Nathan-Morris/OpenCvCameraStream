...
public:
    static void outputSocketLastError(std::basic_ostream<char>& outStream = std::cout);
    static std::string getSocketLastError();
...
public:
    ...
    bool setSockOpt(int level, int option, const OptionValueType& optionValue) {
        return !::setsockopt(
            this->_socket, level, option, (const char*)&optionValue, sizeof(OptionValueType)
        );
    }
